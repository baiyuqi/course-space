...
import com.hbpvu.jec.bookstore.order.repository.CartItemRepository;
import com.hbpvu.jec.bookstore.order.repository.CartRepository;
...
public class CartServiceImpl implements CartService {
	...
	private void fillItems(Cart cartByUserName) {
		List<CartItem> items = cartItemRepository.findByCartId(cartByUserName.getCartId());
        cartByUserName.setCartItems(items);
	}
    
    @Override
    public Cart getCartByCartId(String cartId) {
        Optional<Cart> byCartId = cartRepository.findByCartId(cartId);
        Cart rst  = byCartId.orElseThrow(() -> new RuntimeException("Cart doesn't exist!!"));
        fillItems(rst);
        return rst;
    }
