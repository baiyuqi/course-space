...
/*
	...
	@Override
	public Page<Product> findByCategoryId(String sort, Integer page, Integer size, String cid) {
        //set defaults
        if (size == null || size == 0) {
            size = 20;
        }
        ...
        if (sort == null) {
            ...
            try {
                ...
                Sort.Direction sortDirection = Sort.Direction.fromString(split[1]);
                order = new Sort.Order(sortDirection, split[0]);
                pageable = PageRequest.of(page, size, Sort.by(order));
        ...
        Page<Product> allProducts = productRepository.findByCategoryId(cid, pageable);
