...
public class UserServiceImpl implements UserService {
  ...
  public String createUser(CreateUserRequest createUserRequest) {
	 ...
	 * 
	 * //if role exists add to list and persist, else add to error response persist
	 * valid roles and send // response containing invalid roles.
	 * roleRepository.findByRoleName(roleName).<Runnable>map(role -> () ->
	 * validRoles.add(role.getRoleName())) .orElse(() -> { Error error =
	 * Error.builder() .code("400") .message(roleName + " role doesn't exist")
	 * .build(); errorResponse.getErrors().add(error); }) .run(); });
	 */
    ...
    User user = User.builder()
        ...
        .roles(new HashSet<>(createUserRequest.getRoleNames()))
        .build();
  ...
  public void updateUser(String userId, UpdateUserRequestFromAdmin updateUserRequestFromAdmin) {
    ...
    if(user.getRoles().size()>0){
      ...
      List<String> existingRoles = new ArrayList<String>(user.getRoles());
